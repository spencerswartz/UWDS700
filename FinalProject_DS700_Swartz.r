...
# Calculate MAD
sum(abs(forecast$Incoming.Examinations[-(1:2)] - forecast.mean$fitted[,1]))/(nrow(forecast)-2)
#Calculate MAPE
sum(abs(forecast$Incoming.Examinations[-(1:2)] - forecast.mean$fitted[,1])/forecast$Incoming.Examinations[-(1:2)])/(nrow(forecast)-2)
#Calculate MSE
sum((forecast$Incoming.Examinations[-(1:2)] - forecast.mean$fitted[,1])^2)/(nrow(forecast)-2)
...
#Calculate MAD
accuracy(ARIMAforcast)[1,3]
#Calculate MAPE
accuracy(ARIMAforcast)[1,5]
#Calculate MSE
accuracy(ARIMAforcast)[1,3]^2
